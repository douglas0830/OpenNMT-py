...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    if opt.lm_bias:
        fields, model, model_opt = \
            onmt.model_builder.load_lm_bias_test_model(opt, dummy_opt.__dict__)
    else:
        if len(opt.models) > 1:
    ...
    if opt.lm_bias:
        translator = LMBiasTranslator(model, fields, global_scorer=scorer,
    ...
    def _translate_batch(self, batch, data):
        ...
        for i in range(self.max_length):
            ...
            for j, b in enumerate(beam):
                ...
                lm_dec_states.beam_update(j, b.get_current_origin(), beam_size)
...
class LMBiasTranslator(object):
    ...
    def _translate_batch(self, batch, data):
        ...
        if True:
            lm_out_enc_states, lm_out_memory_bank = self.model.lm_out.encoder(src, src_lengths)
            lm_out_dec_states = self.model.lm_out.decoder.init_decoder_state(
                src, lm_out_memory_bank, lm_out_enc_states)
            lm_in_enc_states, lm_in_memory_bank = self.model.lm_in.encoder(src, src_lengths)
            lm_in_dec_states = self.model.lm_in.decoder.init_decoder_state(
                src, lm_in_memory_bank, lm_in_enc_states)
        #lm_enc_states, lm_memory_bank = self.model.lm_encoder(src, src_lengths)
        #lm_dec_states = self.model.lm_decoder.init_decoder_state(
        #    src, lm_memory_bank, lm_enc_states)
        ...
        if True:
            if isinstance(lm_out_memory_bank, tuple):
                lm_out_memory_bank = tuple(rvar(x.data) for x in lm_out_memory_bank)
            else:
                lm_out_memory_bank = rvar(lm_out_memory_bank.data)
            lm_out_dec_states.repeat_beam_size_times(beam_size)

            if isinstance(lm_in_memory_bank, tuple):
                lm_in_memory_bank = tuple(rvar(x.data) for x in lm_in_memory_bank)
            else:
                lm_in_memory_bank = rvar(lm_in_memory_bank.data)
            lm_in_dec_states.repeat_beam_size_times(beam_size)
        ...
        for i in range(self.max_length):
            ...
            if True:
                lm_out_dec_out, lm_out_dec_states, lm_out_attn = self.model.lm_out.decoder(
                    inp, lm_out_memory_bank, lm_out_dec_states,
                    memory_lengths=memory_lengths,
                    step=i)

                lm_out_dec_out = lm_out_dec_out.squeeze(0)

                lm_in_dec_out, lm_in_dec_states, lm_in_attn = self.model.lm_in.decoder(
                    inp, lm_in_memory_bank, lm_in_dec_states,
                    memory_lengths=memory_lengths,
                    step=i)

                lm_in_dec_out = lm_in_dec_out.squeeze(0)
            ...
            if not self.copy_attn:
                out = self.model.generator.forward(dec_out).data
                lm_out_out = self.model.lm_out.generator.forward(lm_out_dec_out).data
                lm_in_out = self.model.lm_in.generator.forward(lm_in_dec_out).data
                #print(out)
                #print(torch.exp(out))
                #print(lm_in_out-lm_out_out)
                out += 0.1*(lm_in_out-1.0*lm_out_out)
                #out = torch.exp(out)
                
                #out += 0.1*(torch.log(torch.exp(lm_in_out)-torch.exp(lm_out_out)))
                out = unbottle(out)
                # beam x tgt_vocab
                beam_attn = unbottle(attn["std"])
            else:
                out = self.model.generator.forward(dec_out,
                                                   attn["copy"].squeeze(0),
                                                   src_map)
                lm_out_out = self.model.lm_out.generator.forward(lm_out_dec_out,
                                                   attn["copy"].squeeze(0),
                                                   src_map)
                lm_in_out = self.model.lm_in.generator.forward(lm_in_dec_out,
                                                   attn["copy"].squeeze(0),
                                                   src_map)
                out += 0.1 * lm_in_out
                # beam x (tgt_vocab + extra_vocab)
            ...
            for j, b in enumerate(beam):
                b.advance(out[:, j],
                          beam_attn.data[:, j, :memory_lengths[j]])
                dec_states.beam_update(j, b.get_current_origin(), beam_size)
                lm_out_dec_states.beam_update(j, b.get_current_origin(), beam_size)
                lm_in_dec_states.beam_update(j, b.get_current_origin(), beam_size)
